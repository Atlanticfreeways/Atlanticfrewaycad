...
export function ModernNavbar() {
    ...
    return (
        <nav className="fixed top-0 left-0 right-0 h-20 border-b border-white/5 bg-slate-950/70 backdrop-blur-2xl z-50">
            <div className="max-w-7xl mx-auto h-full px-4 md:px-8 flex items-center justify-between">
                ...
                <div className="flex items-center space-x-6">
                    ...
                    <Link href="/auth/register" className="bg-white text-black px-6 py-3 rounded-2xl text-sm font-black hover:bg-slate-200 transition-all active:scale-95 shadow-xl shadow-white/5">
                        Launch Console
                    </Link>
                </div>
            </div>
...
function NavMenuItem({ label, children, isActive, onMouseEnter, onMouseLeave }: any) {
    return (
        <div className="relative" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
            <button className="flex items-center space-x-1 text-sm font-bold text-slate-400 hover:text-white transition-colors py-8">
                <span>{label}</span>
            ...
            <AnimatePresence>
                {isActive && (
                    <motion.div
                        ...
                        className="absolute left-0 top-full -mt-2 bg-slate-900 border border-white/10 rounded-3xl shadow-3xl overflow-hidden"
                    >
                        {children}
                    </motion.div>
                )}
...
export function ModernFooter() {
    return (
        <footer className="bg-slate-950 border-t border-white/5 pt-32 pb-20 relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8">
                ...
                <div className="pt-12 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-8">
                    <div className="flex items-center space-x-8">
                        <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em]">© 2026 Atlantic Financial Inc.</p>
                    ...
                    <div className="flex items-center space-x-8 text-[10px] font-black text-slate-500 uppercase tracking-widest">
...
function SocialLink({ icon: Icon }: any) {
    return (
        <Link href="#" className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-slate-500 hover:text-white hover:bg-white/10 transition-all">
...
function FooterList({ items }: { items: string[] }) {
    return (
        <ul className="space-y-4">
            {items.map(item => (
                <li key={item}>
                    <Link href="/auth/register" className="text-sm font-bold text-slate-500 hover:text-blue-500 transition-colors">{item}</Link>
                </li>
            ))}
        </ul>
    );
